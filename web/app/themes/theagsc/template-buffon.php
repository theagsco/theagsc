<?php
/**
 * Template Name: Buffon
 */
?>
<a href="<?= esc_url(home_url('/')); ?>" class="theagsc_logo"><img class="svg pink" id="pinkBurst" src="<?php echo get_template_directory_uri(); ?>/dist/images/theagsc_small.svg"></a>
<section id="buffonTitleWrapper">
  <p class="islaScript">Introducing</p>
  <h1 class="buffonTitle"><span contenteditable="true">Buffon</span>
    <img class="svg blue" id="blueBurst" src="<?php echo get_template_directory_uri(); ?>/dist/images/buffon_burst.svg">
    <img class="svg pink" id="pinkBurst" src="<?php echo get_template_directory_uri(); ?>/dist/images/buffon_burst.svg">
    <img class="svg red" id="redDrop" src="<?php echo get_template_directory_uri(); ?>/dist/images/buffon_drop.svg">
    <img class="svg yellow" id="yellowDrop" src="<?php echo get_template_directory_uri(); ?>/dist/images/buffon_drop.svg">
    <img class="svg blue" id="blueDrop" src="<?php echo get_template_directory_uri(); ?>/dist/images/buffon_drop.svg">
    <img class="svg pink" id="pinkDrop" src="<?php echo get_template_directory_uri(); ?>/dist/images/buffon_drop.svg">
    <img class="svg white" id="whiteDrop" src="<?php echo get_template_directory_uri(); ?>/dist/images/buffon_drop.svg">
  </h1>
</section>

<section id="blurb">
<p contenteditable="true">A playful reverse-contrast typeface from <strong>The AGSC</strong>.</p>
</section>

<!-- <section id="buyNow">
  <div id="buyText">
    <p class="lettering">$60</p>
  </div>
  <div id="lostType">
    <p class="lettering" id="lt-lost">Lost</p>
    <p class="lettering" id="lt-type">Type</p>
  </div>
</section>

<section id="cloud">
  <img class="svg" src="<?php echo get_template_directory_uri(); ?>/dist/images/buffon_cloud.svg">
</section> -->
